...
Note that we can apply this framework to find the probability of finding the particle with a certain momentum, energy, etc. In those cases, we would use the wavefunction in momentum space or energy space, respectively, since $\psi(x)$ is in position space and therefore does not directly provide momentum or energy information.\newpage\noindent
\textbf{Total Wavefuntion $\mathbf{\Psi(x,t)}$}\\
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Continuous case:
        \[\text{Prob(between $x_m$ and $x_n$)} = \int_{x_m}^{x_n} \Psi^*(x,t)\Psi(x,t)\, dx\]
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Continuous case:
        \[\langle x \rangle = \int_{-\infty}^{\infty} x \Psi^*(x,t)\Psi(x,t)\, dx\]
    \end{itemize}
...
\end{itemize}
Recall that the total wavefunction $\Psi(x,t)$ has time dependence. However, the time dependent term cancels out when we multiply the complex conjugate, so the following statement is equivalent, however we write it: $\Psi^*(x,t)\Psi(x,t) = |\psi(x)|^2$.\\\\
...
\[\boxed{\hat{H}\psi_n = E_n\psi_n,\quad \hat{H} = -\frac{\hbar^2}{2m} \frac{\partial^2}{\partial x^2} + u(x)}\]
This is \textbf{THE most useful equation} for this unit. We will be applying this equation to three different cases (infinite well, finite well, and hydrogen atom) to find the quantized energy levels and corresponding wavefunctions of a particle in those environments.\\\\
Where:
\begin{itemize}
    \item $E_n$ is the energy eigenvalues corresponding to the quantized energy levels of the particle in that environment.
    \item $\psi_n$  is the energy eigenfunctions where $|\psi_n(x)|^2$ gives the probability density of finding the particle at position $x$ given that the particle is in energy state $E_n$.
\end{itemize}
