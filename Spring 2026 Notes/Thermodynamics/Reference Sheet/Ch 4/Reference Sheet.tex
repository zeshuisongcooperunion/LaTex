...
\begin{center}
    ...
    \normalsize Ch 4: Energy Analysis of Closed Systems
\end{center}
...
\[e = u + \frac{1}{2}v^2 + gh \quad \text{(specific total energy)}\]
...
\]
\[
\implies u_2-u_1 = c_{\svol}[T_2-T_1] \quad (\text{constant $ c_{\svol} $ or $ c_p $})
\]
\[
c_p = c_{\svol} + R \; (\text{ideal gas}), \quad
c_p = c_{\svol} = c \; (\text{incompressible substances})
\]
\[Q = mc\Delta T \quad c_\text{water} = 4.18 \, \frac{\text{kJ}}{\text{kg}\cdot\text{K}}\]
\[k=c_p/c_{\svol} \quad \text{(specific heat ratio)}\]
