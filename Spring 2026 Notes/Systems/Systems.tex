...
\begin{itemize}
    ...
    \[\boxed{\frac{Y(s)}{R(s)} = \frac{C(s)P(s)}{1 + C(s)P(s)H(s)} = \frac{\text{(Feedforward TF)}}{1 \pm \text{(Open Loop TF)}}}\]
\end{itemize}
\underline{Note:} The $\pm$ in the closed loop transfer function depends on the sign of the feedback (positive or negative feedback). For negative feedback (subtracting the feedback signal), we use the minus sign; for positive feedback, we use the plus sign.\\\\
